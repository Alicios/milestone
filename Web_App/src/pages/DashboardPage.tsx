...
import { Box, Button, Card, Divider, InputAdornment, Menu, MenuItem, OutlinedInput, Stack, Tooltip, Typography } from '@mui/material'
...
export function DashboardPage() {
  ...
  return <>
    <Stack direction={{ xs: 'column', lg: 'row' }} spacing={{ xs: 2, lg: 5 }} alignItems="stretch">
      <Box flexGrow={1} minWidth={0}>
        <Stack spacing={1.5} mb={3}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 1.5, sm: 0 }} alignItems={{ xs: 'stretch', sm: 'center' }} sx={{ width: '100%', display: { xs: 'flex', sm: 'grid' }, gridTemplateColumns: { sm: 'auto minmax(0, 1fr) auto' }, columnGap: { sm: 1.5 }, rowGap: 1.5 }}>
            <Button onClick={toggleAll} sx={{ flexShrink: 0, bgcolor: 'white', color: 'black', border: '4px solid black', borderRadius: '34px', px: 3, py: 1.25, fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: '1.2rem', '&:hover': { bgcolor: '#f5f5f5' } }}>{allExpanded ? 'Collapse All' : 'Expand All'}</Button>
            ...
            <Stack direction="row" spacing={1.5} sx={{ flexGrow: { sm: 1 }, justifyContent: { sm: 'flex-end' }, alignSelf: { xs: 'flex-end', sm: 'center' } }}>
              <Button startIcon={<SortIcon />} onClick={(event) => setSortMenuAnchor(event.currentTarget)} sx={{ bgcolor: 'white', color: 'black', border: '4px solid black', borderRadius: '34px', px: 3, py: 1.25, fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: '1.2rem', '&:hover': { bgcolor: '#f5f5f5' } }}>Sort by</Button>
              <Button startIcon={<FilterListIcon />} onClick={(event) => setFilterMenuAnchor(event.currentTarget)} sx={{ bgcolor: statusFilter === 'all' ? 'white' : '#eb681d', color: statusFilter === 'all' ? 'black' : 'white', border: '4px solid black', borderRadius: '34px', px: 3, py: 1.25, fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: '1.2rem', '&:hover': { bgcolor: statusFilter === 'all' ? '#f5f5f5' : '#d15a17' } }}>Filter</Button>
            </Stack>
          </Stack>
