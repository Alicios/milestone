...
import { Box, Button, Card, Collapse, Divider, Fade, Grow, InputAdornment, Menu, MenuItem, OutlinedInput, Stack, Tooltip, Typography, useTheme } from '@mui/material'
...
function PatientRow({ patient, expanded, onToggle, assignments, onAssign, onDischarge }: PatientRowProps) {
  ...
  return <Card sx={{ bgcolor: 'transparent', boxShadow: 'none', border: 0, overflow: 'visible', mb: 1.5 }}>
    <Button fullWidth onClick={onToggle} aria-expanded={expanded} sx={{ minHeight: 64, justifyContent: 'center', position: 'relative', bgcolor: '#91c8c0', color: accentText, border: `4px solid ${border}`, borderRadius: '40px', fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: { xs: '1.2rem', sm: '1.65rem' }, transition: 'transform 180ms ease, box-shadow 180ms ease, background-color 180ms ease', '&:hover': { bgcolor: '#82bdb5', transform: 'translateY(-2px)', boxShadow: '0 8px 0 rgba(0, 0, 0, 0.18)' }, '&:active': { transform: 'translateY(0)' }, '&.Mui-focusVisible': { outline: '3px solid #eb681d', outlineOffset: 3 } }}>
      {patient.name}<ExpandMoreIcon sx={{ position: 'absolute', right: 12, bgcolor: isDark ? '#0e252d' : 'black', color: '#91c8c0', borderRadius: '50%', transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 220ms ease' }} />
    </Button>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Stack direction="row" spacing={{ xs: .25, sm: .75 }} justifyContent="center" sx={{ minWidth: 616 }}>
          {days.map((day, index) => {
            ...
            return <Box key={day} sx={{ flex: 1, minWidth: 0, textAlign: 'center', display: 'flex', flexDirection: 'column' }}>
              <Typography sx={{ bgcolor: index === 2 ? '#eb681d' : neutralSurface, color: 'white', border: `3px solid ${border}`, borderRadius: '28px', py: { xs: 1, sm: 1.5 }, fontFamily: 'Georgia, serif', fontStyle: 'italic', fontWeight: 700, fontSize: { xs: '.65rem', sm: '1.15rem' } }}>{day}</Typography>
              <Box sx={{ mt: .5, bgcolor: isAssignable ? assignableSurface : surface, border: `3px solid ${border}`, borderRadius: '24px', minHeight: { xs: 112, sm: 132 }, flexGrow: 1, display: 'flex', alignItems: routine ? 'flex-start' : 'center', justifyContent: 'center', p: .5 }}>
                {routine ? <Grow in key={routine.id} timeout={220}><Typography sx={{ width: '100%', minWidth: 0, bgcolor: '#91c8c0', color: accentText, border: `2px solid ${border}`, borderRadius: '18px', px: .5, py: .75, fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: { xs: '.8rem', sm: '.9rem' }, lineHeight: 1.3, whiteSpace: 'normal', overflowWrap: 'anywhere' }}>{routine.name}</Typography></Grow>
                  : isAssignable ? <Button
                    onClick={() => onAssign(index)}
                    aria-label={`Assign routine to ${patient.name} on ${weekdayNames[index]}`}
                    sx={{ minWidth: 0, width: '100%', p: .5, display: 'flex', flexDirection: 'column', gap: .5, color: text, borderRadius: '18px', fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: { xs: '.75rem', sm: '.85rem' }, lineHeight: 1.2, '&:hover': { bgcolor: isDark ? '#35545c' : '#c8deda' }, '&.Mui-focusVisible': { outline: `3px solid ${border}`, outlineOffset: -2 } }}
                  >
                    <Box component="span" sx={{ width: { xs: 42, sm: 48 }, height: { xs: 42, sm: 48 }, flexShrink: 0, borderRadius: '50%', bgcolor: '#4b9da9', border: `3px solid ${border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><AddIcon sx={{ color: 'white', fontSize: { xs: 30, sm: 36 } }} /></Box>
                    <Box component="span">Assign<br />Routine</Box>
                  </Button>
                    : <Tooltip title={status}><Box sx={{ width: { xs: 42, sm: 66 }, height: { xs: 42, sm: 66 }, flexShrink: 0, borderRadius: '50%', border: `3px solid ${border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: status === 'missed' ? '#ff333c' : status === 'complete' ? '#8bd153' : status === 'modified' ? '#ffdc55' : neutralSurface, color: 'white' }}><StatusIcon status={status} /></Box></Tooltip>}
              </Box>
            </Box>
          })}
        </Stack>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', pt: 2.5, px: { xs: 1.5, sm: 2 }, pb: 2 }}>
          <Button
            ...
            sx={{ bgcolor: surface, color: text, border: `3px solid ${border}`, borderRadius: '24px', px: 2, py: .75, fontFamily: 'Georgia, serif', fontStyle: 'italic', '&:hover': { bgcolor: isDark ? '#29454d' : '#e8ddba' } }}
    ...
    <Fade in appear timeout={320}>
      ...
      <Box flexGrow={1} minWidth={0}>
        <Stack spacing={1.5} mb={3}>
          ...
          {visiblePatients.length ? visiblePatients.map((patient) => <PatientRow key={patient.id} patient={patient} expanded={expanded.includes(patient.id)} onToggle={() => togglePatient(patient.id)} assignments={assignments[patient.id] ?? {}} onAssign={(dayIndex) => setAssignmentTarget({ patient, dayIndex })} onDischarge={() => setDischargeTarget(patient)} />) : <Card sx={{ p: 5, textAlign: 'center', border: `4px solid ${border}`, bgcolor: surface, color: text }}><Typography variant="h6" fontFamily="Georgia, serif">No patients found</Typography></Card>}
        </Stack>
      </Box>
