...
interface Patient { id: string; name: string; statuses: Array<'missed' | 'complete' | 'modified' | 'none' | 'routine'> }
...
function PatientRow({ patient, expanded, onToggle }: { patient: Patient; expanded: boolean; onToggle: () => void }) {
  return <Card sx={{ bgcolor: 'transparent', boxShadow: 'none', border: 0, overflow: 'visible', mb: 1.5 }}><Button fullWidth onClick={onToggle} aria-expanded={expanded} sx={{ minHeight: 64, justifyContent: 'center', position: 'relative', bgcolor: '#91c8c0', color: 'black', border: '4px solid black', borderRadius: '40px', fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: { xs: '1.2rem', sm: '1.65rem' }, '&:hover': { bgcolor: '#82bdb5' } }}>{patient.name}<ExpandMoreIcon sx={{ position: 'absolute', right: 12, bgcolor: 'black', color: '#91c8c0', borderRadius: '50%', transform: expanded ? 'rotate(180deg)' : 'none' }} /></Button>{expanded && <Box sx={{ bgcolor: '#4b9da9', border: '4px solid black', borderTop: 0, borderRadius: '0 0 28px 28px', mt: -2, pt: 3, px: { xs: .5, sm: 1 }, pb: 1 }}><Stack direction="row" spacing={{ xs: .25, sm: .75 }} justifyContent="center">{days.map((day, index) => <Box key={day} sx={{ flex: 1, minWidth: 0, textAlign: 'center' }}><Typography sx={{ bgcolor: index === 2 ? '#eb681d' : '#777777', color: 'white', border: '3px solid black', borderRadius: '28px', py: { xs: 1, sm: 1.5 }, fontFamily: 'Georgia, serif', fontStyle: 'italic', fontWeight: 700, fontSize: { xs: '.65rem', sm: '1.15rem' } }}>{day}</Typography><Box sx={{ mt: .5, bgcolor: 'white', border: '3px solid black', borderRadius: '24px', height: { xs: 76, sm: 112 }, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Tooltip title={patient.statuses[index]}><Box sx={{ width: { xs: 42, sm: 66 }, height: { xs: 42, sm: 66 }, borderRadius: '50%', border: '3px solid black', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: patient.statuses[index] === 'missed' ? '#ff333c' : patient.statuses[index] === 'complete' ? '#8bd153' : patient.statuses[index] === 'modified' ? '#ffdc55' : '#777777', color: 'white' }}><StatusIcon status={patient.statuses[index]} /></Box></Tooltip></Box></Box>)}</Stack></Box>}</Card>
}
